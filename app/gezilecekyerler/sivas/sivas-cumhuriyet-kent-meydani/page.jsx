...
export const metadata = {
  title: "Sivasgurun.com - Sivas Cumhuriyet Kent Meydanı",
  description: "Sivas Cumhuriyet Kent Meydanı, Selçuklu, Osmanlı ve Cumhuriyet dönemlerine ait önemli yapıları barındıran, şehrin tarihî ve kültürel kalbidir. Çifte Minareli Medrese, Buruciye Medresesi ve Atatürk Kongre Müzesi gibi eserlerle adeta açık hava müzesini andırır.",
};
export default async function SivasKentMeydani(){
    const getPostData = async () => {
    try {
        const sql = `SELECT * FROM gezilecekyerler WHERE idgezilecekyerler=7`;
    ...
    return(
       <section className='mt-12 max-w-screen-lg w-full mx-auto'>    
            <article className='flex flex-col gap-8'>
                <figure>
                    <Image className='m-auto rounded-lg shadow-lg shadow-slate-500 w-full' width={500} height={500} src={`/${reslt[0].content.mainImg}`} alt="sivas-cumhuriyet-kent-meydani" />
                    <figcaption className='text-center dark:text-textDark text-textLight'>Cumhuriyet Kent Meydanı, Sivas</figcaption>
                </figure>
		...
		if(i === 2){
                ...
                {/*<figure className="p-2">
                      <iframe src={reslt[0].content.position} width='600' height='450' title="Sivas Cumhuriyet Kent Meydanı Güzergah" className="max-w-screen-lg w-full mx-auto" allowfullscreen='' loading='lazy' referrerpolicy='no-referrer-when-downgrade'></iframe>
